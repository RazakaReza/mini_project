...
const {authenticateToken, authorizeRole} = require("./middleware/middleware");
...
app.put("/movies/:id", authenticateToken,(req, res) => {
  console.log('Request UPDATE /movies/:id oleh user:', req.user.username);
  const { title, director, year } = req.body;
  const sql =
...
app.post("/directors", authenticateToken, (req, res) => {
  console.log('request CREATE /directors/:id oleh user:', req.user.username);
...
app.put("/directors/:id", [authenticateToken, authorizeRole("admin")],(req, res) => {
  console.log('request UPDATE /directors/:id oleh user:', req.user.username);
  const { name, birthYear } = req.body;
...
app.post('/auth/register-admin', (req, res) => {
  ...
  bcrypt.hash(password, 10, (err, hashedPassword) => {
    if (err) {
      return res.status(500).json({ error: 'Terjadi kesalahan hashing password' });
    }
